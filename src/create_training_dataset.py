...
def create_combined_dataset(output_path="space5_training_data.csv", include_epluszsz=True):
    ...
    if include_epluszsz:
        ...
        if epluszsz_info and epluszsz_info["is_daily_data"] and epluszsz_info["space5_columns"]:
            ...
            print(f"Saved SPACE5-1 sizing data to space5_sizing_data.csv: {space5_data.shape[0]} rows, {space5_data.shape[1]} columns")
    ...
    general_cols = [
        'timestep', 'month', 'day_of_month', 'hour', 
        'obs_outdoor_temperature', 'obs_outdoor_humidity', 'obs_wind_speed', 'obs_wind_direction',
        'obs_diffuse_solar_radiation', 'obs_direct_solar_radiation',
        'action_Heating_Setpoint_RL', 'action_Cooling_Setpoint_RL', 'reward_reward'
    ]
    ...
    if space5_cols:
        ...
        other_cols = [
            ...
            if col not in selected_cols and 
            not any(f'SPACE{i}' in col for i in range(1, 5)) and
            'zone' not in col.lower()
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="space5_training_data.csv", 
                      help="Path to save the output CSV file")
    parser.add_argument("--skip-epluszsz", action="store_true", 
                      help="Skip processing epluszsz.csv")
