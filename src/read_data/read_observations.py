...
def validate_observations(file_path):
    ...
    expected_columns = [
        ...
        "wind_speed", "wind_direction", "diffuse_solar_radiation", 
        "direct_solar_radiation", "htg_setpoint", "clg_setpoint", "air_temperature", 
        "air_humidity", "people_occupant", "co2_emission", 
    ...
    if 'HVAC_electricity_demand_rate' in df.columns:
        ...
        print(f"HVAC electricity demand range: min={df['HVAC_electricity_demand_rate'].min()}, max={df['HVAC_electricity_demand_rate'].max()}")
