...
class VisionModelValidator:
        ...
        try:
            ...
            if hasattr(config, "model_type"):
                if config.model_type == "vision-encoder-decoder" or config.model_type == "trocr":
                    ...
                    logger.warning("Unknown model type: %s. Will try to infer from architecture.", config.model_type)
        ...
        try:
            ...
            if self.model_type == "trocr":
                ...
                if hasattr(config, "encoder") and config.encoder:
                    ...
                    logger.info("  Vision Number of layers: %s", config.encoder.get("num_hidden_layers", "unknown"))
                    logger.info("  Vision Number of attention heads: %s", config.encoder.get("num_attention_heads", "unknown"))
                ...
                if hasattr(config, "decoder") and config.decoder:
                    ...
                    logger.info("  Text Number of layers: %s", config.decoder.get("decoder_layers", "unknown"))
                    logger.info("  Text Number of attention heads: %s", config.decoder.get("decoder_attention_heads", "unknown"))
                ...
                if hasattr(config, "vision_config") and config.vision_config:
                    ...
                    logger.info("  Vision attention heads: %s", vision_config.get("attention_heads", "unknown"))
                ...
                if hasattr(config, "text_config") and config.text_config:
                    ...
                    logger.info("  Text attention heads: %s", text_config.get("num_attention_heads", "unknown"))
            ...
            if self.model_type == "trocr" and model_parameters < 1e8:
                ...
                logger.warning("Llama Vision model has fewer parameters than expected (%s)", f"{model_parameters:,}")
    ...
    def validate_tensor_properties(self):
        ...
        try:
            ...
            for name, param in tqdm([(n, p) for n, p in self.model.named_parameters() if n in sample_layers], 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", type=str, choices=["trocr", "mllama"], default=None,
